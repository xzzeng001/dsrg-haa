...
# add two-qubit gate
def add_two_qubit_gates_1(circ: QuantumCircuit,
                        q_reg: QuantumRegister,
                        network: Any,
                        params: Any,
                        iparams: Any) -> Tuple[QuantumCircuit,int]:
   ...
   for i in range(network[0]-1):
      for j in [i+1]:
         circ.rx(params[iparams], q_reg[i])
         circ.cnot(i,j)
         circ.rx(params[iparams+1], q_reg[j]) 

         circ.rz(params[iparams+2], q_reg[i])
         circ.cnot(i,j)
         circ.rz(params[iparams+3], q_reg[j]) 
...
# add two-qubit gate
def add_two_qubit_gates_2(circ: QuantumCircuit,
                        q_reg: QuantumRegister,
                        network: Any,
                        params: Any,
                        iparams: Any) -> Tuple[QuantumCircuit,int]:

   for i in [0,2,3,4,5]: #range(network[0]):
