...
def circuit(params):
   ...
   for icycle in range(ncycle):
      ...
      neuron_params = params[nparams:]
      tx, ty, tz = neuron_params[0:3]
      qml.IsingXX(2*tx, wires=(0,network[0]-1))
      qml.IsingYY(2*ty, wires=(0,network[0]-1))
      qml.IsingZZ(2*tz, wires=(0,network[0]-1))
      nparams += 3
...
#    noise_model.add_all_qubit_quantum_error(error_1, ['cx'])
    ...
    #dev2=qml.device('qiskit.aer', wires=n_qubits_tot, noise_model=noise_model)
    @partial(
    qml.transforms.mitigate_with_zne,
    scale_factors=[1., 2., 3.],
    folding=fold_global,
    extrapolate=poly_extrapolate,
    extrapolate_kwargs={'order' : 1},)
    @qml.qnode(dev1)
